  ...
  <v-dialog v-model="active" persistent max-width="600" scrollable @keydown.esc.stop="onCancel">
    <v-card>
      <v-card-title class="fc-card-title">
        <h2 class="subheading">Criar empresa</h2>
      </v-card-title>
      <v-card-text>
        <v-layout row wrap>
          <v-flex sm12>
            <v-form ref="empresaCreation" @submit.prevent>
...
export default {
  ...
  watch: {
    active () {
      this.$refs.empresaCreation.reset()
    }
  },

  mounted () {
    if (!window.getApp._events.CREATE_CATEGORY) {
      window.getApp.$on('CREATE_COMPANY', async (value) => {
  ...
  methods: {
    ...
    async onSubmit () {
      if (this.$refs.empresaCreation.validate()) {
        this.loading = true
        // Format fields
        this.company.telefone = this.$utils.formatPhoneNumber(this.company.telefone)
        this.company.status = !!this.company.status

        const response = await this.createCompany({ data: this.company, auth: this.auth})

        if (response.data) {
          this.resetCompany()
          this.loading = this.active = !response
          window.getApp.$emit('COMPANY_CREATED', true)
        } else {
          this.loading = false
